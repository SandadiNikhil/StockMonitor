...
def run_stock_monitor(symbol):
    ...
    earnings_growth_data = fmp_api.get_earnings_growth_rate(
        symbol)  # Fetch earnings growth data

    if company_name is None:
        print(
            f"Warning: Could not retrieve company name for symbol: {
                symbol.upper()}.")  # Warning if company name is None

    if real_time_price_data is None:
        # Warning if real-time price is None
        print(
            f"Warning: Could not retrieve real-time price data for {symbol.upper()}.")

    if historical_data is None:
        print(
            f"Warning: Could not retrieve historical price data for {
                symbol.upper()}.")  # Warning if historical data is None

    if key_ratios is None:
        print(
            f"Warning: Could not retrieve key ratios for {
                symbol.upper()}. Fundamental scores will be limited.")  # Warning if key ratios are None

    if earnings_growth_data is None:
        print(
            f"Warning: Could not retrieve Earnings Growth Rate data for {
                symbol.upper()}. This metric will be excluded from scoring.")  # Specific warning for earnings growth
    ...
    if key_ratios and len(key_ratios) > 0:
        ...
        weighted_scores['weighted_pe_score'] = ratio_scores['pe_score'] * \
            formula.WEIGHTS_5_YEAR["pe_ratio"]
        weighted_scores['weighted_pb_score'] = ratio_scores['pb_score'] * \
            formula.WEIGHTS_5_YEAR["pb_ratio"]
        weighted_scores['weighted_de_score'] = ratio_scores['de_score'] * \
            formula.WEIGHTS_5_YEAR["de_ratio"]
        weighted_scores['weighted_roe_score'] = ratio_scores['roe_score'] * \
            formula.WEIGHTS_5_YEAR["roe"]
    ...
    if earnings_growth_data:
        # Assuming API now returns growth rate as a float directly
        earnings_growth_rate_value = earnings_growth_data
        earnings_growth_score = formula.score_earnings_growth_rate(
            earnings_growth_rate_value)  # Score it
        weighted_earnings_growth_score = earnings_growth_score * \
            formula.WEIGHTS_5_YEAR["earnings_growth"]  # Apply weight
    else:
        earnings_growth_score = None
        weighted_earnings_growth_score = None
    ...
    # Call display function in display.py
    display.display_stock_data(display_data)
...
def main():
    ...
    stock_symbol = input(
        "Enter the stock symbol you want to monitor (e.g., AAPL, TSLA): ").strip().upper()
